<template>
  <div>
    <div class="leader">
			<ul>
				<li :class="{active: this.orderType === 'all'}" @click="getOrderListData('', 'all')">全部订单</li>
				<li :class="{active: this.orderType === 'payed'}" @click="getOrderListData(1, 'payed')">已付款</li>
				<li :class="{active: this.orderType === 'pickup'}" @click="getOrderListData(3, 'pickup')">待取货</li>
				<li :class="{active: this.orderType === 'confirm'}" @click="getOrderListData(5, 'confirm')">已完成</li>
			</ul>
    </div>
    <div class="order-item" v-for="item of list" :key="item.orderId" @click="toPage(item)">
			...
			<div class="item-more">
				<div
					...
					v-show="item.orderStateId === 1 || item.orderStateId === 3 || item.orderStateId === 4">
						<span>取消订单</span>
				</div>
				<div class="btn" @click.stop="evaluate(item)" v-show="item.orderStateId === 5">
					<span>评价</span>
				</div>
				<div class="btn" @click.stop="changeOrderStateData(item, 5)" v-show="item.orderStateId === 4 || item.orderStateId === 3">
					<span>确认收货</span>
				</div>
			</div>
    </div>
...
export default {
	...
	methods: {
		// 获取订单列表
		getOrderListData (orderStateId = '', type) {
			let data = {
		...
		changeOrderStateData (item, changeState) {
			req('changeOrderState', {
				...
				if (data.code === 0) {
					...
					this.getOrderListData('', this.orderType)
				} else {
