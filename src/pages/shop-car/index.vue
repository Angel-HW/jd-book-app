  ...
  <div class="container">
    ...
    <div class="close-count">
      ...
      <div>
        ...
        <button @click="addOrderData">结算</button>
        <button @click="deleteShopCarData">删除</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    deleteShopCarData () {
      ...
      this.$confirm('此操作将永久删除商品,是否继续?').then(() => {
        req('deleteShopCar', {
          shopCartId: shopCartIds
        }).then(data => {
          console.log(data)
    ...
    getTotal () {
      ...
      this.list.forEach(item => {
        if(item.checked === true) {
          total += parseFloat(item.goodsPrice * item.cartGoodsCount)
        }
      })
